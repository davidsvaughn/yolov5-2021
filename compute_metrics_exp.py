'''
Experimental version... trying mask_iou for semantic classes
'''
...
def mask_iou(p_mask, t_mask):
    intersection = (p_mask * t_mask).sum()
    if intersection == 0:
        return 0.0, 0.0, 0.0
    union = np.logical_or(p_mask, t_mask).sum()
    p = intersection/p_mask.sum()
    r = intersection/t_mask.sum()
    return intersection/union, p, r
...
# pred_dir = '/home/david/code/phawk/data/solar/indivillage/models/model1/detect/exp3/labels'

pred_dir = '/home/david/code/phawk/data/solar/indivillage/models/model2/detect/exp3/labels'
    ...
    if nl:
        ...
        for cls in np.unique(tcls_tensor):
            ...
            if pi.shape[0]:
                ...
                ##########################################
                ## mask_iou - semantic classes
                # if cls in [4,5,6,7,8,9]:
                #     w = 1000
                #     M1,M2 = np.zeros((w, w)), np.zeros((w, w))
                #     for b in boxes1:
                #         bb = (w*b).round().astype(np.int32)
                #         M1[bb[0]:bb[2], bb[1]:bb[3]] = 1
                #     for b in boxes2:
                #         bb = (w*b).round().astype(np.int32)
                #         M2[bb[0]:bb[2], bb[1]:bb[3]] = 1
                #     miou, P, R = mask_iou(M1, M2)
                #     print(miou)
                ###########################################
