...
def load_classifiers(names, device):
    model_root = '/home/david/code/phawk/data/fpl/damage/rgb/resnet/python'
    models = adict()
...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            imc = im0.copy() if (opt.save_crop or classify) else im0  # for opt.save_crop
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if classify:  # Apply ResNet classifiers to cropped images
                        c = int(cls)
                        if c not in models:
                            continue
                        ...
                        # cv2.imwrite(f'/home/david/code/phawk/data/fpl/damage/rgb/detect/{models[c].name}.jpg', img_crop)
                        ################
                        ...
                        # print(mu)
                        ######################################
                        damaged = modelc(torch.unsqueeze(img_crop, 0).to(device)).argmax(1).cpu().numpy()
                        print('DAMAGED' if damaged==1 else 'NORMAL')
                        print('')
